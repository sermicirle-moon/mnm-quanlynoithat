...
<div class="flex-1 overflow-y-auto p-8 bg-[#f8f9ff] text-slate-900">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-5 mb-8">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5"><p class="text-xs font-bold uppercase tracking-wider text-slate-500">Doanh thu đã thanh toán</p><p class="text-2xl font-bold mt-2 text-emerald-700"><?php echo esc_html(number_format((float) ($summary['total_revenue'] ?? 0))); ?>đ</p><p class="text-xs text-slate-400 mt-2"><?php echo esc_html((string) ($summary['completed_invoice_count'] ?? 0)); ?> hóa đơn</p></div>
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5"><p class="text-xs font-bold uppercase tracking-wider text-slate-500">Chi phí nhập</p><p class="text-2xl font-bold mt-2 text-rose-700"><?php echo esc_html(number_format((float) ($summary['stock_in_cost'] ?? 0))); ?>đ</p><p class="text-xs text-slate-400 mt-2"><?php echo esc_html((string) ($summary['stock_in_count'] ?? 0)); ?> phiếu đã nhập</p></div>
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5"><p class="text-xs font-bold uppercase tracking-wider text-slate-500">Chênh lệch thu - nhập</p><p class="text-2xl font-bold mt-2 text-slate-900"><?php echo esc_html(number_format((float) ($summary['gross_profit'] ?? 0))); ?>đ</p><p class="text-xs text-slate-400 mt-2"><?php echo esc_html((string) ($summary['profit_rate'] ?? 0)); ?>% trên doanh thu</p></div>
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5"><p class="text-xs font-bold uppercase tracking-wider text-slate-500">Giá trị tồn hiện tại</p><p class="text-2xl font-bold mt-2 text-amber-700"><?php echo esc_html(number_format((float) ($summary['inventory_value'] ?? 0))); ?>đ</p><p class="text-xs text-slate-400 mt-2"><?php echo esc_html(number_format((int) ($summary['inventory_quantity'] ?? 0))); ?> sản phẩm tồn hiện tại</p></div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <div class="flex justify-between items-center mb-6"><h2 class="text-xl font-bold">Xu hướng tăng trưởng doanh thu</h2><span class="text-xs font-bold text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full">Hóa đơn đã thanh toán</span></div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8">
        ...
        <?php $renderTable('Trạng thái giao trên hóa đơn', 'Tình trạng giao hàng ghi trực tiếp trên hóa đơn.', ['Trạng thái', 'Số lượng', 'Giá trị'], $shippingStatuses, static function ($row) { ?><tr><td class="px-6 py-4 font-bold"><?php echo esc_html($row['status'] ?? ''); ?></td><td class="px-6 py-4"><?php echo esc_html((string) ($row['count'] ?? 0)); ?></td><td class="px-6 py-4"><?php echo esc_html(number_format((float) ($row['amount'] ?? 0))); ?>đ</td></tr><?php }); ?>
        <?php $renderTable('Trạng thái xử lý phiếu xuất kho', 'Theo dõi luồng vận chuyển từ phiếu xuất kho.', ['Trạng thái', 'Số phiếu', 'Phí VC'], $stockOutStatuses, static function ($row) { ?><tr><td class="px-6 py-4 font-bold"><?php echo esc_html($row['status'] ?? ''); ?></td><td class="px-6 py-4"><?php echo esc_html((string) ($row['count'] ?? 0)); ?></td><td class="px-6 py-4"><?php echo esc_html(number_format((float) ($row['shipping_fee'] ?? 0))); ?>đ</td></tr><?php }); ?>
    </div>

    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-8">
        <?php $renderTable('Cảnh báo tồn kho hiện tại', 'Sản phẩm có tồn hiện tại thấp hơn hoặc bằng 5.', ['Mã', 'Sản phẩm', 'Tồn hiện tại', 'Trạng thái'], $stockAlerts, static function ($row) { ?><tr class="hover:bg-rose-50/60"><td class="px-6 py-4 font-bold text-rose-700"><?php echo esc_html($row['ma_sp'] ?? ''); ?></td><td class="px-6 py-4"><p class="font-bold"><?php echo esc_html($row['ten_sp'] ?? ''); ?></p><p class="text-xs text-slate-400"><?php echo esc_html($row['category'] ?? ''); ?></p></td><td class="px-6 py-4 font-bold"><?php echo esc_html((string) ($row['so_luong_ton'] ?? 0)); ?></td><td class="px-6 py-4"><?php echo esc_html($row['trang_thai'] ?? ''); ?></td></tr><?php }); ?>
